...
def thresold_calculation(output):
   
    safe_images, unsafe_images = 0,0


    for key , value_dict in output.items():
        keymax = max(value_dict, key=value_dict.get) 
        # print(f"image path : {key} and contains {keymax}")
        if keymax == 'sexy' or 'porn' == keymax:
            unsafe_images+=1
        else:
            safe_images+=1    

    print("Number of safe images" , safe_images)
    print("Number of unsafe images", unsafe_images)
    unsafe_thresold = 30
    Number_of_images = len([*output])
    prob = unsafe_images/Number_of_images*100

    if prob >= unsafe_thresold:
        print("Given URL is not safe for work where probability is" , prob)
    else:
        print('Given URL is safe for work where probability is')

# def line_arguments(saved_model_path , image_source , image_dim=224):

#     model = load_model(saved_model_path)   
#     print(model) 
#     image_preds = classify(model, image_source,image_dim)

#     thresold_calculation(image_preds)
